 ...
 *  @Using LiDAR sensor to detect object
 *  Linear Approach
...
public class DBScan {
	...
	// le minimum de point pour qu'un point fasse partied'un cluster
	int minPts;
	
	// Minpts par defaut, si un minimun de point n'est pas donnee au lancement du programmme
	final public static int DEFAULT_MINPTS=12;
	
	// le rayon du cercle autour du point servant a definir la region de voisinage
	Double eps;
	
	// epsilone par defaut, si un rayon eps n'est pas donnee au lancement du programmme
	...
	public static void main(String[] args) {
		...
		// Si aucun argument n'a ete detecte lors du lancement du programme, Une liste vide par default sera utilisee.
			if ((args.length==0)) {
		 ...
		 * Si aucune valeurn'a ete rentrer pour epsilone lors du lancement du programme,une valeur par default sera utilisee.
		 * Si une valeur negative est rentre pour epsilone lors du lancement du programme,une valeur par default sera utilisee.
		 */
		try {
			...
			System.out.println("Vous n'avez pas defini eps lors du lancement du programe , alors une valeur eps = 0.8 est utilisee par defaut pour ce test.");
			scan.setEps(DEFAULT_EPS);
		
		}catch (NumberFormatException e) {
        	System.out.println("Le format que vous avez rentre pour eps n'est pas valide, alors une valeur eps = 0.8 est utilisee par defaut pour ce test.");
            scan.setEps(DEFAULT_EPS);
        }
		catch (Exception e)
        {
            if (Double.parseDouble(args[1])<0)
            	System.out.println("Le rayon ne peut pas etre negative, alors une valeur eps = 0.8 est utilisee par defaut pour ce test.");
                scan.setEps(DEFAULT_EPS);
        }
		/*
		 * Verifie la validite de minPts avant de le laisser passer
		 * Si aucune valeur n'a ete rentrer pour minPts lors du lancement du programme,une valeur par default sera utilisee.
		 * Si une valeur negative est rentre pour epsilone lors du lancement du programme,une valeur par default sera utilisee.
		 */
		...
		try {
			...
			System.out.println("Vous n'avez pas defini MinPts lors du lancement du programe, une valeur MinPts = 12 sera utilisee par defaut.");
			scan.setMinPts(DEFAULT_MINPTS);
		}
		catch (NumberFormatException e) {
        	System.out.println("Le format que vous avez rentre pour minPts n'est pas valide, une valeur MinPts = 12 sera utilisee par defaut.");
            scan.setEps(DEFAULT_EPS);
        }
		catch (Exception e)
        {
            if (Double.parseDouble(args[2])<0) {
            	System.out.println("Le nombre de points ne peut pas etre negatif, une valeur MinPts = 12 sera utilisee par defaut.");
                scan.setMinPts(DEFAULT_MINPTS);
            }
        }
		...
		String saveName=fileName+"_clusters_"+scan.eps+"_"+scan.minPts+"_"+scan.getNumberOfClusters()+".csv";
		...
		// arrange la liste contenant le size de chaque cluster du plus grand au plus petit.
		Arrays.sort(clusterSize);
		
		for(int k=clusterSize.length-1;k>0;k--) {
			if (aMap.containsKey(String.valueOf(clusterSize[k]))) {
				System.out.println("Cluster "+aMap.get(String.valueOf(clusterSize[k]))+" have:"+clusterSize[k]+" points.");
			}
	...
	/*
	 * Methode pour sauvegarder les references aux clusters a leur quantite respective
	 * @param clustersSize
	 ...
	 * @param p
	 * @param colors
	 * @return un String contenant les coordonnees d'un point, son label et les valeurs R G B de sa couleur
	 */
	public String toString(Point3D p, List<Double[]> colors) {
		
		    Double[] ptColour= colors.get(p.label);
			return  p.getX()+","+p.getY() + "," + p.getZ()+ ","+ p.label +","+ptColour[0]+","+ptColour[1]+","+ptColour[2];
	}
